...
,[userInfo])
  ...
  <div className="flex flex-col items-center justify-center px-6 py-8 mx-auto md:h-screen lg:py-0">
    <a href="#" className="flex items-center mb-6 text-4xl font-semibold text-white dark:text-white">
      <Image
    ...
    <div className="w-full bg-spheri-black rounded-lg shadow dark:border border md:mt-0 sm:max-w-md xl:p-0 dark:bg-spheri-black dark:border-spheri-grey border-spheri-grey">
      <div className="p-6 space-y-4 md:space-y-6 sm:p-8">
        ...
        <form className="space-y-4 md:space-y-6" onSubmit={handleSubmit}>
          <div>
            ...
            <input type="text" name="username" id="username" value={formData.username} onChange={handleInputChange} className="bg-spheri-black-black border border-gray-600 text-white sm:text-sm rounded-lg focus:outline-none block w-full p-2.5 dark:bg-spheri-black-black-black dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-gray-600 dark:focus:order-gray-600 dark:placeholder-spheri-medium-grey" placeholder="pick a username" required={true} />
          </div>
          <div>
            <label htmlFor="email" className="block mb-2 text-sm font-medium text-white dark:text-white">Your email</label>
            <input type="email" name="email" id="email" value={formData.email} onChange={handleInputChange} className="bg-spheri-black-black border border-gray-600 text-white sm:text-sm rounded-lg focus:outline-none block w-full p-2.5 dark:bg-spheri-black-black-black dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-gray-600 dark:focus:order-gray-600 dark:placeholder-spheri-medium-grey" placeholder="name@company.com" required={true} />
          </div>
          <div>
            <label htmlFor="password" className="block mb-2 text-sm font-medium text-white dark:text-white">Password</label>
            <input type="password" name="password" value={formData.password} onChange={handleInputChange} id="password" placeholder="••••••••" className="bg-spheri-black-black border border-gray-600 text-white sm:text-sm rounded-lg focus:outline-none block w-full p-2.5 dark:bg-spheri-black-black-black dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-gray-600 dark:focus:order-gray-600 dark:placeholder-spheri-medium-grey" required={true} />
          </div>
          <div>
            <label htmlFor="confirmPassword" className="block mb-2 text-sm font-medium text-white dark:text-white">Confirm password</label>
            <input type="password" name="confirmPassword" value={formData.confirmPassword} onChange={handleInputChange} id="confirmPassword" placeholder="••••••••" className="bg-spheri-black-black border border-gray-600 text-white sm:text-sm rounded-lg focus:outline-none block w-full p-2.5 dark:bg-spheri-black-black-black dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-gray-600 dark:focus:order-gray-600 dark:placeholder-spheri-medium-grey" required={true} />
          </div>
          ...
          <p className="text-sm font-light text-gray-500 dark:text-gray-400">
            Already have an account? <a href="Login" className="font-medium text-spheri-violet-button hover:underline dark:text-spheri-violet-button">Login here</a>
          </p>
        </form>
      </div>
    </div>
